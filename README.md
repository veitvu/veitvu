# Veit Vu
**Full-stack Developer** | *Ho Chi Minh City, Vietnam* | [veitvu@outlook.com](mailto:veitvu@outlook.com) | [LinkedIn](https://linkedin.com/in/veitvu) | [GitHub](https://github.com/veitvu)

## 🚀 Overview
Experienced Full-stack Developer with over 8 years in .NET and 5 years managing complex database systems. I specialize in crafting scalable applications using C#, Python, Java, React, and Solana blockchain development, with expertise in microservices, database optimization, ERP solutions, and DeFi applications. Passionate about delivering impactful tech solutions across traditional and blockchain environments.

## 💎 Core Values & Professional Philosophy
As a technology professional, I am guided by these fundamental principles:

- **Technical Excellence**: I believe in mastering my craft through continuous learning and deliberate practice. Every line of code I write reflects my commitment to quality and best practices.
- **Adaptability & Innovation**: Technology evolves rapidly, and I embrace this change. I actively pursue new technologies and methodologies, seeing them as opportunities rather than challenges.
- **User-Centric Focus**: Behind every technical problem is a human need. I approach development with empathy for end-users, creating solutions that are not just functional but intuitive and valuable.
- **Collaborative Leadership**: I value open communication and knowledge sharing within teams. My approach to leadership focuses on empowering team members while maintaining technical rigor.
- **Bridge Builder**: I excel at connecting traditional business needs with cutting-edge technologies, particularly in bridging conventional systems with blockchain innovations.
- **Sustainable Development**: I design systems with long-term maintainability and scalability in mind, balancing immediate needs with future growth requirements.

## 🛠️ Technical Skills
| Category | Skills |
|----------|--------|
| **Languages** | C#, Python, Java, JavaScript, TypeScript, React, Rust, PHP, C/C++ |
| **Frameworks** | .NET Core, React, Next.js, Spring Boot, FastAPI, ASP.NET, Angular, Vue |
| **Databases** | PostgreSQL, SQL Server, Oracle, MySQL, MongoDB, SQLite |
| **DevOps & Cloud** | Docker, Redis, Azure, Jenkins, Kubernetes, Kibana |
| **Blockchain** | Solana, Anchor Framework, Web3.js, Rust (Smart Contracts), GraphQL API |
| **Tools** | Git, GitHub, VS Code, Visual Studio, Jira, Trello |

## 💼 Professional Experience
### DeFi Lending Platform
- Architected and developed a full-stack DeFi lending platform on Solana blockchain.
- Implemented secure smart contracts for lending pools, borrowing, staking, and governance.
- Created a modular React/Next.js frontend with comprehensive features for DeFi operations.
- **Tech**: Rust, Anchor Framework, React, Next.js, TypeScript, GraphQL, Solana Web3.js

### Lecturer (Programming & AI Research)
- Taught programming and researched AI with large language models (CodeLlama, GPT-2).
- **Tech**: C, C#, PHP, SQL, Python, HuggingFace

### Senior Developer (ERP Systems)
- Built an accounting module on Odoo ERP, automating financial workflows.
- **Tech**: C#, SQL Server, GitLab

### Senior Developer (CRM Solutions)
- Developed a CRM system with chatbot and ticket management for e-commerce.
- **Tech**: Python (FastAPI, PyTorch), React, PostgreSQL, Docker, Redis

### Senior Developer (Production Planning)
- Created a web tool for material and delivery scheduling in manufacturing.
- **Tech**: Java, Spring Boot, Oracle, Jenkins

### Team Leader (Software Projects)
- Led development of ERP modules and a pharmacy tender platform for international clients.
- **Tech**: C#, .NET Core, Angular, SQL Server, Azure

### Junior Developer (Embedded Systems)
- Worked on IoT projects including digital weight systems and RFID inventory.
- **Tech**: C Embedded, Arduino, RS232 Protocol

## 🌟 Key Projects
### Solana Lending Pool DeFi Platform
*Full-stack decentralized finance platform enabling lending, borrowing, and staking on Solana blockchain.*
- **Tech**: Rust, Anchor Framework, React, Next.js, TypeScript, Tailwind CSS
- **Features**: 
  - Decentralized lending pools with dynamic interest rates
  - Collateralized borrowing with health factor monitoring
  - Token staking with rewards distribution
  - DAO governance for protocol parameters
  - Real-time price feeds and liquidation risk management
- **Architecture**:
  - Modular React components following atomic design principles
  - Context-based state management with reducers
  - Internationalization support
  - Comprehensive analytics and risk assessment tools
- **Impact**: Created a secure, user-friendly DeFi platform with comprehensive financial tools

### Odoo ERP Accounting App
*Accounting module to manage customers, invoices, and payments.*
- **Tech**: C#, SQL Server, GitLab
- **Impact**: Automated financial workflows

### CRM with Chatbot in E-Commerce
*CRM system with chatbot and ticket management features for e-commerce.*
- **Tech**: Python (FastAPI, PyTorch), React, PostgreSQL, Docker
- **Impact**: Enhanced customer support automation

### Production Planning Software
*Web tool for material and delivery scheduling in manufacturing.*
- **Tech**: Java, Spring Boot, Oracle, Jenkins
- **Impact**: Improved planning accuracy

### ERP Warehouse & Accounting
*ERP system for inventory and financial management.*
- **Tech**: Java, Spring Boot, Oracle, Bootstrap
- **Impact**: Streamlined warehouse operations

### Pharmacy Tender Platform
*Bidding platform for pharmacy equipment suppliers.*
- **Tech**: C#, .NET Core, Angular, Azure, SQL Server
- **Impact**: Simplified bidding processes

## 🧠 Professional Approach
I approach software development as both a science and an art. My methodology combines:
- **First-principles thinking** to break complex problems into solvable components
- **Systems-level architecture** considering both immediate function and future scalability
- **Cross-domain knowledge application** to bring the best practices from various technology areas
- **Balanced technical debt management** to deliver timely solutions while maintaining code quality

## 📬 Contact
- **Email/Slack**: [veitvu@outlook.com](mailto:veitvu@outlook.com)
- **LinkedIn**: [Veit Vu](https://linkedin.com/in/veitvu)
- **GitHub**: [veitvu](https://github.com/veitvu)
